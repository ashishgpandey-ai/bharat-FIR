...
# firbot_with_gemini.py - FIR Bot with ₹10 QR Payment & Gemini AI Drafting
...
# ============================================================
# UPI PAYMENT CONFIGURATION - ₹10 ONLY
# ============================================================

UPI_CONFIG = {
    'upi_id': 'ashishgpandey@ybl',  # Your UPI ID
...
def generate_qr_code_with_amount(amount=10, reference=None):
    """Generate QR code with amount ₹10 prominently displayed"""
    try:
...
def generate_fir_application_with_gemini(fir_data, language='en'):
    ...
    Returns:
        str: Professionally drafted FIR application
    """
    if not gemini_model:
    ...
    try:
        ...
        if language == 'hi':
            ...
            6. अंत में शपथ पत्र (Affidavit) शामिल हो
            """
        else:
            prompt = f"""
            ...
            6. Includes an affidavit at the end
            """
...
def generate_fir_application_template(fir_data, language='en'):
    ...
    if language == 'hi':
        ...
        {fir_data.get('incident_location', '_____________')} में 
        {fir_data.get('description', '_____________')} घटना घटी।
        ...
        मैं, {fir_data.get('complainant_name', '_____________')}, 
        सत्य शपथ लेता/लेती हूँ कि उपर्युक्त विवरण सत्य एवं सही हैं 
        तथा मेरे द्वारा दी गई जानकारी पूर्णतः सही है।
        ...
        I, {fir_data.get('complainant_name', '_____________')}, 
        do hereby solemnly affirm and declare that the above information 
...
class SimpleDB:
    ...
    def load(self):
        if os.path.exists(self.filename):
            try:
                ...
                return {'firs': [], 'users': {}, 'feedback': [], 'payments': []}
        return {'firs': [], 'users': {}, 'feedback': [], 'payments': []}
    ...
    def get_stats(self):
        ...
        total_amount = len([f for f in self.data['firs'] if f.get('payment_status')]) * FIR_AMOUNT
        return {
            'total': total, 
            'paid': paid, 
            'pending': pending,
...
# [All command handlers remain the same as previous version]
...
async def confirm_fir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'confirm_yes':
        ...
        # Generate FIR application using Gemini
        await query.edit_message_text(
        ...
        # Store FIR application in database
...
# ============================================================
# PDF GENERATOR WITH APPLICATION
# ============================================================

def generate_pdf_with_application(fir_data, language):
    """Generate PDF with FIR and Application"""
    try:
...
def main():
    """Run the bot"""
    print("=" * 60)
    print("🤖 COMPLETE FIR BOT WITH GEMINI AI")
    print("=" * 60)
